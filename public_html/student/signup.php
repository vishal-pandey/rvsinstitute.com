...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#signup_button").click(function(){
	        ...
	        $.post("signupcontrol.php",send,function(data,status){
	        	$("#content").css("display","block");
	        	var op = JSON.parse(data);
	        	if (op.error == "") {
		        	$("#content").html('<form  method="post" id="otp-form"><div class="input-group"><span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span><input id="otp" type="text" class="form-control" name="otp" placeholder="Enter The Otp Serd to your mobile no." required="true"></div></form>');
		        	$("#otp_submit").css("display","block");
		        	$(".rotp").css("display","block");


		        	$("#vishal").css("display","block");
	        		$("#vishal").html(op.otpp);
	        	}
	        	else{
		        	$("#content").html(op.error);
	        	}
	        });
		});

		$("#otp_submit").click(function(){
			var data1 = $("#otp-form").serializeArray();
			function objectifyForm(formArray) {//serialize data1 function
		          var returnArray = {};
		          for (var i = 0; i < formArray.length; i++){
			            returnArray[formArray[i]['name']] = formArray[i]['value'];
		          }
	        ...
	        send1 = objectifyForm(data1);
	        finalsend = merge_options(send,send1);
	        $.post("otpcontrol.php",finalsend,function(data1,status){
	        	var op = JSON.parse(data1);
	        		if (op.error == "") {
	        			$("#content").html("Succes");
	        			window.location.href="./student.php";
	        		}else{
		        		$("#vishal").css("display","block");
		        		$("#vishal").html(op.error);
		        		$("#content").html('<form  method="post" id="otp-form"><div class="input-group"><span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span><input id="otp" type="text" class="form-control" name="otp" placeholder="Enter The Otp Serd to your mobile no." required="true"></div></form>');
		        	$("#otp_submit").css("display","block");
		        	$(".rotp").css("display","block");
	        		}
	...
	<div class="col-sm-6 well signup">
		...
		<form method="post" id="signup-form">
			...
			<div class="input-group">
		      ...
		      <input id="mobile" type="number" class="form-control" name="mobile" placeholder="Mobile No." required="true">
			</div>
			<div class="input-group">
		      <span class="input-group-addon"><i class="glyphicon glyphicon-education"></i></span>
		      <input id="class" type="text" class="form-control" name="class" placeholder="Class / Course / Subject" required="true">
			</div>
			<div class="input-group">
			...
			<div class="input-group">
		      <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
		      <input id="r_password" type="password" class="form-control" name="r_password" placeholder="Re Enter Password" required="true">
			</div>
			<div class="input-group">
			...
			<div class="input-group formcolor">
		...
		<center>
			<button type="submit" class="btn btn-primary col-sm-offset-" id="signup_button">Sign Up</button>
		</center>
		<br>
		<div class="well" id="content" style="display: none;">
			
		</div>
		<center><!-- <button type="submit" class="rotp btn btn-primary pull-left" id="signup_button" style="display: none;">Re-Send OTP</button> --><button type="submit" class="btn btn-primary col-sm-offset-" id="otp_submit" style="display: none;">Submit</button></center>
		<div class="well" id="vishal" style="display: none; margin-top: 15px;"></div>
